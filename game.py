...
from detect_frequency import find_frequency
...
while running:
    ...
    interval = find_frequency()

    # while index < len(time_stamp):
    # elapsed = time.time() - start_time
    # if interval[index] - 50 < millisecond_conversion(elapsed) < interval[index] + 50:
    if interval[index] - 50 < count < interval[index] + 50:
        print('ayo')
        red_or_blue = random.randint(0, 1)
        # 0 represents red, 1 represents Blue
        if red_or_blue == 0:
            print(f'RED... {interval[index]}')
            note = Note(SCREEN_WIDTH, SCREEN_HEIGHT - 150, 5, note1_image, 'note1')
        else:
            print(f'BLUE... {interval[index]}')
